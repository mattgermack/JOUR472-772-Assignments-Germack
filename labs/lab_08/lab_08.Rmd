...
**A1.** Yes, there is definitely a difference. Two of the highest months for calls are November and December, which is directly contradictory to the thesis of "there are more calls in warmer months". Similarly to the other chart, though, January and February are on the low end of calls in a month.

```{r}
calls_by_month <- pg_overdose_calls |> 
  group_by(month) |> 
  summarize(total = n()) |> 
  mutate(pct = (total / sum(total) * 100))

calls_by_month |>
  ggplot() + geom_bar(aes(x = month, weight = pct)) +
  labs(
    title = '% of PG County overdose calls by month',
    y = '% of calls',
    x = 'Month',
    caption = 'Data from Prince Georges County EMS'
  ) +
  coord_flip()
```
...
**A2.** PG County overdose calls by week is a bimodal dataset, with substantial peaks in weeks 28 and 51. The first four weeks of the year are relatively low in total.

```{r}
calls_by_week <- pg_overdose_calls |>
...
calls_by_week |>
  ...
  labs(
    title = 'Total PG County overdose calls by week',
    y = 'Total calls per week',
    x = 'Week',
    caption = 'Data from Prince Georges County EMS'
  )
```
...
**A3.** Most of the zip codes with high amounts of 911 overdose calls are relatively lower-income, and most of the higher-income zip codes have low amounts of 911 overdose calls. This data shows that there is something causing those in worse areas to be more affected by drug overdoses, and further reporting for this story would look into what those things may be.

```{r}
calls_by_zip <- pg_overdose_calls |>
  group_by(zipcode) |>
  summarize(total_calls = n())

income_by_zip <- get_acs(geography = "zcta", variables = "B19013_001", year = 2022)

calls_by_zip_and_income <- calls_by_zip |>
  left_join(income_by_zip, by = c("zipcode" = "GEOID"))

calls_by_zip_and_income |>
  ggplot(aes(x = estimate, y = total_calls, label = zipcode)) +
  geom_point() +
  geom_text(check_overlap = TRUE, size = 2, vjust = -1) +
  labs(
    title = 'PG County overdose calls by zip code cross-referenced with median income',
    x = 'Median income',
    y = 'Overdose calls',
    caption = 'Data from Prince Georges County EMS and US Census Bureau ACS')
