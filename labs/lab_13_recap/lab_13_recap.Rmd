...
childcare_counties <- childcare_inspections |> left_join(md_counties, join_by(fips == GEOID))
```

##### A1: NAMELSAD is best to use, because it differentiates Baltimore City and Baltimore County.
...
childcare_counties <- childcare_counties |>
  mutate(percentage = overdue / sum(overdue) * 100)

# Create the chart with ggplot
childcare_counties |>
  ggplot() +
  geom_bar(aes(x=reorder(NAMELSAD,percentage), weight=percentage)) +
  theme(
    axis.text.x = element_text(angle = 45,  hjust=1)
  )
```

##### **A2:** Montgomery County has by far the highest percentage of overdue childcare facility inspections, at 36.8%
...
md_county_acs <- get_acs(geography="county", variables = "B06001_002", state='MD', year=2022)
...
```{r}
# Join the dataframes
childcare_counties_acs <- childcare_counties |> left_join(md_county_acs, join_by(fips == GEOID))

# Calculate the rates for children per facility
childcare_counties_acs <- childcare_counties_acs |>
  mutate(rate = children_pop / total)

```

##### **A3:** Garrett County has the most children per facility, with 87.5.
...
```{r}
#Google told me to install sf package + do this because for whatever reason it said I didn't have geometry in the dataset when I tried to run the code below this
childcare_counties_acs <- st_as_sf(childcare_counties_acs)

ggplot() +
  geom_sf(data = childcare_counties_acs, aes(fill = rate), color = NA) +
  scale_fill_viridis_c(option = "magma") +
  theme_minimal()

```

##### **A4**: Not particularly; there are some major rural hotspots for too many children per facility, but this problem persists in Baltimore/Anne Arundel/PG County as well.
...
md_income_acs <- get_acs(geography="county", variables = "B19013_001", state='MD', year=2022)
...
md_income_acs <- md_income_acs |>
  rename(median_income = estimate)
```

```{r}
# Join the dataframes
income_childcare_acs <- childcare_counties_acs |> left_join(md_income_acs, join_by(fips == GEOID))

```

```{r}
# Create a scatterplot with median income and rate of children per facility
income_childcare_acs |>
  ggplot(aes(x = median_income, y = rate, label = NAMELSAD)) +
  geom_point() +
  geom_text(check_overlap = TRUE, size = 2, vjust = -1) +
  labs(
    title = 'MD Counties by Income Children per Childcare Facikity ',
    x = 'Median income',
    y = 'Children per childcare facility',
    caption = 'Data from US Census Bureau ACS')
```

```{r}
# Create a correlation test.
cor.test(income_childcare_acs$median_income, income_childcare_acs$rate)

```

##### A5: There is no meaningful correlation between median income and children per childcare facility, due to the .6067 p-value being way higher than any reasonable level for statistical significance.
...
##### A6: The frequent use of "emergency bag" indicates there's often issues with those; additionally, multiple top bigrams including "missing" gives us an idea of what the issues commonly were.
