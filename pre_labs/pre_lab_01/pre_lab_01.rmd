...
**Answer: 79,366 observations of 9 variables**
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: chr(characters) and dbl(doubles)**
...
In the space below, write the name of the business from the first row of the "name" column. **Answer: there is no name column, but first title is Directed Research**
...
**Answer: First line calls umd_courses dataset, with the |> "pipe" command. second line groups the data points by term, third line calls for a summary table and fourth line creates a column called count_classes and calls the n function to count how many classes there are assigned to each term**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: The terms are now sorted by # of classes in descending order; term with most classes at top, term with fewest at bottom**
...
**Answer: There is now a row for each department's class count within each term; data is more split up**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: We've removed the department separation, and now we have added a line of code to add up the total seats of all classes combined in each term. That number is displayed in the third column**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: We've now added two more columns that show mean and median seats per class in each term. Those variables are established in lines 202 and 203**
...
**Answer: We're no longer categorizing by term, we're categorizing by department. As with the previous question, we're still counting the total number of classes by department, the total amount of seats + median and mean seats in classes by department.**
...
**Answer:** 
**1. Which departments have the most massive-lecture-style classes? We can determine this by finding which departments' seat means are the largest compared to their medians**
**2. Which departments have the fewest seats relative to total # of classes? Why?**
**3. What's the deal with the music performance department???**
