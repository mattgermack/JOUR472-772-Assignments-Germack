...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **Answer**All words are capitalized and dates are in chronological scraping order + in characters instead of r's date format.
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** Author, summary, page views if accessible, category(?)
...
tags there are in the HTML source? **Answer**There aren't any listed ul tags in this data, only li
...
tags. Describe the output: **Answer**A list of what are presumably clickable options on the website followed by an HTML list of press releases on the site.
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer**The same list that was in HTML now in a one-column R list.
