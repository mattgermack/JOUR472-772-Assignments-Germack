...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** County names are not standardized, there's dots in the county names, and columns are not properly labeled.
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** Columns are still not properly labeled and there's a bunch of dots in each county name.
...
**Task** Run the codeblock below. What else do we need to do? **Answer** The biggest thing is to remove the dots from county names; as far as I can tell, everything else is good.
