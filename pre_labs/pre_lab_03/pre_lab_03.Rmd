...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No it does not, because we've increased the guess_max to the actual amount of rows in the dataset.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The "Amount" field is listed as "dbl", which are numerical values designed to include decimals. The "date" column is listed in characters instead of the lubridate "Date" type we learned in the previous lab.
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** Within md_grant_loans, grants are sorted by fiscal year and category, then summarized in the count of # of grants/loans. Then they are arranged by fiscal year. Counting up *total* number of grants vs *total amount* of grants could be problematic.
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** There are two loans, from the DNR and MDOE, that are smaller than $100.
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** It makes column names lowercase and replaces spaces with underscores
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** 174 creates new dataset, 175 initiates clean_names, 176 renames grantor column to source, 177 calls mutate command to make all characters uppercase within source, grantee and description
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** 27 instances of duplicates, 58 total duplicate rows
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** A new column labeled zip5 removes the -XXXX from all applicable zip codes by cutting the character strings after 5 characters
...
group_by(zip5) |>
  summarise(tourism_total = sum(amount)) |>
  arrange(desc(tourism_total))
```
