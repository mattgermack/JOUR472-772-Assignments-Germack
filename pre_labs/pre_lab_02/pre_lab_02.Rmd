...
**Answer** Because failing to make a new variable would require altering/deleting columns and data from the original dataset, which we should not do under virtually any circumstance. We may need other columns at another point.
...
**Answer:** In the data section on top-right of RStudio, the expenditure_date variable has been changed from character(chr) to Date. The dates change format to the mdy format we choose in the mutate command, with dashes instead of backslashes
...
**Answer** All dates are brought down to a common day of month - in this case the 1st, which is likely easiest for the computer to do - so that they can be grouped into months easiest, and the computer doesn't tweak out over differences in days of months. But since the Date data format still needs to display a day of month, it does.
...
**Answer** Within the mutate command, two variables are being created: total_votes, which is made by adding up each candidates' votes in each county, and pct_moore, which is the amount of votes Moore got in each county divided by that county's total votes.
...
**Answer** Moore's vote share is now displayed as a percentage, because we multiplied the proportion by 100, and the counties are ordered in descending order by the highest percentage of votes that Moore got.
...
**Answer** It depends on what we're using the data for. But it's the exact same dataset, but just flipped.
...
**Answer** str_squish removes errant spaces from beginning/end of strings and extra spaces between words ————— str_dup duplicates the selected string. Found at https://stringr.tidyverse.org/reference/index.html
...
**Answer** The code is looking to detect strings of characters that are also names of states, then creating a new variable called "state" with corresponding abbreviations of states depending on which state is in the address.
...
**Answer** Yes - this table displays total value of expenditures from each state, and total value of expenditures from all other states/localities combined as N/A
...
1.  What was the most common purpose of expenditure in each state?

2.  What are the largest expenditures from outside Maryland?

3.  What are the smallest expenditures? Why are they expenditures?
