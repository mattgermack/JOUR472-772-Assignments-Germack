...
**Task** Load the five CSV files. Do they have the same column structure? **Answer** 2022 has lowercase total instead of uppercase total
...
**Task** Load the Maryland 2020 county population data **Answer** Done
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** 24 rows, which is how many there should be.
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **Answer: It says 'by = join_by(county)' meaning it's joined by county**
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** PG County was 60.9% Black and voted 78.3% Democratic, highest in both categories. Generally speaking, the higher the Black population, the higher the Democratic vote(with Montgomery an expected outlier due to other factors)
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard County is the highest Asian population at 19.37% and voted 52.38% Dem. There is far less correlation, if any, between Asian population and county voting results.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer** It's now a character
