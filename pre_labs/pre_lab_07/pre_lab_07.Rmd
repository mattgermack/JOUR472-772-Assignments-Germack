...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Answer** Nevada had the largest percent change.
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Answer** Howard County has the highest median household income, by a margin outside the MOE.
...
The headline should be "Estimated Median household income by county in Maryland"
```{r}
md |>
...
Is the difference between Calvert and Montgomery significant? **Answer** No, because they share much of the same range of error.

Is the difference between Howard and everyone else significant? **Answer** Yes, its MOE has no overlap with any other MOE's
...
**Task** Let's look at the average household income for high schools that have 60% or more graduates immediately enrolling in college, and then look at those with less than 40%. Write a sentence describing the results. **Answer** Household income for high schools with >60% graduation rate is substantially higher than high schools with <40% graduation rates(111k vs 69k)
...
**Task** What is another question you could ask of this data? **Answer** Among the outliers to this trend, are there confounding variables that could explain why they are outlier high schools?
